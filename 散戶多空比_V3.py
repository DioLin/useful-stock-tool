...
##定義資料庫位置
conn = sqlite3.connect('/Users/dio/Desktop/stock-ML/useful-stock-tool-master-2/stockdb.db') # ~代表路徑
c = conn.cursor()
...
while len(data) < n_days:
    ...
    if isTraddingDay(queryDate):
        ...
        data[index]=[queryDate,indiviualPlayer_ratio,major_trader_ratio,foreign_investment_long]
         # sql insert 存入 sqlite 語法
        c.execute('insert into stockplot(date,indiviualPlayer_ratio, major_trader_ratio,foreign_investment_long) values (?,?,?,?)', data[index])
        conn.commit()
        index=index+1
